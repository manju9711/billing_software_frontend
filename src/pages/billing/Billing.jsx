...
//   const effectiveTotal   = total - advanceUsed;          // cash needed after advance
//   const totalCovered     = received + advanceUsed;       // cash + advance
//   const balance          = totalCovered - total;         // + = change/extra  − = still owed
...
//       credit_limit:    c.credit_limit    || 0,
//       points:          c.loyalty_points  || 0,
//       advance_balance: parseFloat(c.advance_balance) || 0,  // ← NEW
//     });

//     const adv = parseFloat(c.advance_balance) || 0;
//     const pts = c.loyalty_points || 0;
//     const msgs = [];
//     if (pts > 0)  msgs.push(`${pts} loyalty points`);
//     if (adv > 0)  msgs.push(`₹${adv.toFixed(2)} advance balance`);
//     showToast(
//       msgs.length > 0
//         ? `Customer has ${msgs.join(" & ")}`
//         : "Customer selected",
//       "success"
//     );
...
//   const handleNameSearch = (value) => {
//     setCustomer(c => ({ ...c, name: value, id: null, credit_enabled: "0", advance_balance: 0 }));
//     clearTimeout(nameSearchTimer.current);
//     if (!value || value.length < 2) { setNameSuggestions([]); return; }
//     nameSearchTimer.current = setTimeout(async () => {
//       const user = JSON.parse(localStorage.getItem("user"));
...
//   const handlePhoneSearch = (value) => {
//     const digits = value.replace(/\D/g, "").slice(0, 10);
//     setCustomer(c => ({ ...c, phone: digits, id: null, name: c.id ? "" : c.name, credit_enabled: "0", advance_balance: 0 }));
//     setPhoneSuggestions([]);
//     clearTimeout(phoneSearchTimer.current);
//     if (digits.length < 3) return;
//     phoneSearchTimer.current = setTimeout(async () => {
...
//     // For non-credit: check that either cash received > 0 OR advance covers it fully
...
//         gst_type:       billType === "gst_bill" ? "with_gst" : "without_gst",
//         gst_no:         billType === "gst_bill" ? customer.gst_no : "",
//         paid_amount:    paymentMethod === "credit" ? 0 : received,  // raw cash received
//         payment_method: paymentMethod,
//         payment_type:   paymentMethod === "credit" ? "credit" : "cash",
//       });
...
//                 onChange={e => setBillType(e.target.value)}
//                 style={{
//                   background:"rgba(255,255,255,.95)", fontWeight:700,
//                   color: billType === "gst_bill" ? "#d97706" : "#059669",
//                   border:"2px solid rgba(255,255,255,.4)",
//                 }}
//               >
...
//                       <div>
//                         <span style={{ fontWeight:700, color: balance < 0 ? "#dc2626" : "#059669", fontSize:14 }}>
//                           {balance < 0 ? "Balance Due" : "Change / Extra"}
//                         </span>
//                         {advanceUsed > 0 && (
//                           <div style={{ fontSize:11, color:"#94a3b8", marginTop:2 }}>
//                             Advance ₹{advanceUsed.toFixed(2)} + Cash ₹{received.toFixed(2)}
//                           </div>
//                         )}
//                       </div>
//                       <div style={{ textAlign:"right" }}>
...
//                       }}>
//                         {[
//                           { label:"Invoice", val:`₹${total.toFixed(2)}`, color:"#312e81" },
//                           { label:"Advance used", val:`₹${advanceUsed.toFixed(2)}`, color:"#059669" },
//                           { label:"Cash received", val:`₹${received.toFixed(2)}`, color:"#4338ca" },
//                         ].map((item, idx) => (
//                           <div key={idx} style={{
//                             background:"rgba(255,255,255,.7)", borderRadius:8, padding:"6px 4px",
//                           }}>
